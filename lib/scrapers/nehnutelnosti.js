...
async function scrape({ location, priceMin, priceMax, type, page, deep }) {
  ...
  // Build URL
  // Nehnutelnosti.sk moved to /vysledky/{cat}/{location}/predaj with ?page=N pagination.
  // Legacy /{cat}/predaj/{location}/ URLs 301-redirect there but DROP query params
  // (including pagination), so we must hit the canonical URL directly.
  let url = `https://www.nehnutelnosti.sk/vysledky/${cat}/`;
